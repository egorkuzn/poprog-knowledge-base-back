...
- `SPRING_PROFILES_ACTIVE=dev` задает профиль окружения;
...
- `AUTH_DEV_HEADERS_ENABLED=true` временно оставляет возможность тестовой авторизации через заголовки.
...
- Система должна позволять временную тестовую авторизацию через dev-заголовки до завершения Keycloak-интеграции.
